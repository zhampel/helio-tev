...
static float4 GetField(float4 pos, __global float* datax, __global float* datay, __global float* dataz)
{
    ...
    int idx = xbin + 100*(ybin+100*zbin);
    //int idx = xbin + 10*(ybin+10*zbin);
    //if (idx > 999 || idx < 0)
    //if (idx > 100*100*100 || idx < 0)
    if (idx > 100*100*100 || idx < 0)
        idx = 0;
    ...
    //B.y = 0.;//datay[idx];
    //B.z = BMAG;//dataz[idx];
    //printf("%i %.07f %.04f %i %.07f %.07f %.07f\n",idx,pos.x,pos.x*m_to_au,xbin,datax[idx],datay[idx],dataz[idx]);
    B.x = datax[idx];
    B.y = datay[idx];
    B.z = dataz[idx];
    //B.x = 1e-6*datax[idx];
                                ...
                                float time_step)
{
    ...
    //printf("%i %.05f %.05f %.05f\n",gid,datax[gid],datay[gid],dataz[gid]);
